...
Field shapes are real; the numbers and digests below are illustrative and do not
come from one actual receipt (the quickstart's own row reads 148 / 81 / 117 / 50
bytes for pre / post / dropped / introduced).

```json
{
  "kind": "compaction_receipt",
...
- **`receipt_digest`** covers the deterministic core (`schema`, `pre`, `post`,
  `dropped`, `introduced`, `compactor`, `method`), so an edit to any of those
  is caught even when the row has been copied *out* of its ledger. It does
  **not** cover `ts` or `opened_at`: out of the ledger the timestamps are
  unattested, and a backdated copy of a row passes `verify_receipt`. Inside
  the ledger, the chain covers the whole row, timestamps included.
...
```python
verify_receipt(row)
# {"ok": True, "self_consistent": True, "content": "skipped",
#  "schema": "v2", "understatement_check": "full",
#  "verified_scope": "bounded_no_content",
#  "notes": ["bounded scan: no content supplied, so nothing was recomputed ..."]}

verify_receipt(row, pre_content=pre, post_content=post)
# {"ok": True, "self_consistent": True, "content": "match",
#  "schema": "v2", "understatement_check": "full",
#  "verified_scope": "full", "notes": []}
```

**Read `verified_scope`, not only `ok` (0.1.4).** `ok` says nothing that was
checked contradicted the row. `verified_scope` says how much was checked, and
the two greens above are not the same green: the first recomputed *nothing* and
the second recomputed *everything*. Only `verified_scope: "full"` means every
claimed number was reproduced from content you hold. A bounded pass now also
carries a note saying what it skipped, because an empty `notes` list on a
bounded scan reads exactly like a clean full verification — which is the whole
failure this field exists to prevent. (`ok` itself is still a plain bool for
compatibility; arcaeon-ledger returns `ok=None` for the same situation, and
aligning the two is an open 0.2.0 question, not a silent change.)
...
#                    | "bounded_no_content"  (+ "+v1_lower_bound" when live),
#  "notes": [...]}
```
...
v0.1.5. Library + packaged self-test, tested against the planted-drop lie,
in-row edits, multiset duplicates, byte totals, schema v1/v2 compatibility,
the understated-dropped-bytes-behind-an-introduction attack, malformed and
hostile rows, and lifecycle misuse (`test_compact.py`,
`test_hypothesis_compact.py`, `test_hardening.py`). Extracted from the context-compaction flow of a
