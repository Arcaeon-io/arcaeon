...
silently, versus one that says out loud what it lost. (An unsealed receipt is
a plain dict, and `verify_receipt()` checks its shape, not its provenance;
tamper-evidence comes from sealing it onto a ledger, next.)

Chain a receipt onto a tamper-evident ledger (optional — this is the only
place `arcaeon-ledger` is required; the digest helpers use it when it happens
to be installed and otherwise compute the identical format with stdlib):
...
The server keeps its own call record (0.1.7): every `tools/call`, success or
refusal, appends one hash-chained row (tool, server timestamp, sha256 of the
arguments, outcome) to `$ARCAEON_CALL_RECORD`, default `./distill.calls.jsonl`.
`python -m arcaeon_distill.mcp_server --verify-calls` walks the chain. Same
chain format as arcaeon-ledger, so that package's verifier reads it too. It
proves what the server said it did; it does not prove the distill was right.
...
dependencies. `arcaeon-ledger` is optional, only required by
