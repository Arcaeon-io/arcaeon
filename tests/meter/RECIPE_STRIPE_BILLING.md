...
```
{"event": "meter.grant", "key_id": "178e09b09ead", "key_hash": "178e09b09ead...<full sha256>", "plan": "pro", "month": "2026-08", "used": 1, "cost": 1, "ts": "2026-08-15T01:04:12Z", "chain": "24b93e7ffb68834ee10121d43cc1b9f6"}
{"event": "meter.grant", "key_id": "178e09b09ead", "key_hash": "178e09b09ead...<full sha256>", "plan": "pro", "month": "2026-08", "used": 2, "cost": 1, "ts": "2026-08-15T01:04:12Z", "chain": "2412996c9472c27766405977156f2e6c"}
...
{"event": "meter.grant", "key_id": "178e09b09ead", "key_hash": "178e09b09ead...<full sha256>", "plan": "pro", "month": "2026-08", "used": 7, "cost": 1, "ts": "2026-08-15T01:04:12Z", "chain": "a110d92c0795f2d96262fb39ceeee502"}
```
...
#              breaks=0, verified_scope='full', declared_breaks=0, declared=[])
```

`ok=True` **together with** `verified_scope="full"` means every row's hash
chains to the one before it — nobody edited, reordered, or deleted a grant
event after the fact without breaking the chain at that exact row.

**Check both fields, not just `ok`.** Since arcaeon-ledger 0.5.7 the verdict is
three-valued and its scope rides in-band, precisely so a partial scan cannot
mint a green:

| verdict | meaning for an invoice |
|---|---|
| `ok=True`, `verified_scope="full"` | every row checked, chain intact. Bill it. |
| `ok=None` (falsy), `verified_scope="bounded_*"` | **no fault found, but the scan did not cover every row** — unchained pre-chain rows were skipped, or a break was excused by a `chain_break_declared` row. Not a green. Look before you bill. |
| `ok=False` | a real break. `first_break` names the first one; **`breaks` is the count — read it, because there may be more than one.** |

`ok=None` is falsy on purpose, so an `if result.ok:` gate fails safe. A gate
written as `if result.ok is not False:` does not — do not write that one.

`rows=7` matching the invoice's `quantity=7` is the receipt — **net of voids**,
see below: the number on the Stripe invoice is the count of
individually-chained, individually-timestamped grant events, not a number that
could have been typed in or silently adjusted. That's the sell — not "trust our
metering," but "here's the chain, verify it yourself."
...
Run it per key — filter on **`key_hash`**, the full sha256 the grant rows have
carried since 0.1.4, *not* `key_id`: that prefix is 48 bits and two customers
can share one, which would silently sum two customers' grants into one
reconciliation total (the exact defect 0.1.2 removed from the counting path).
Any mismatch is then a real event worth finding, not noise. If the void append fails as well — the same dying disk,
...
This recipe and its example script ride the repo, not the wheel — they are not
importable package code and do not by themselves move `arcaeon_meter`'s
version. (That version is whatever `arcaeon_meter.__version__` reports; this
file no longer names a number, because the last one it named went stale for
four releases and a doc that states a stale fact is worse than one that states
none.) If a future recipe needs a code change — a helper baked into the
package — that is a real version bump with its own CHANGELOG entry.
