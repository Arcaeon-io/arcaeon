...
from arcaeon.save.meter.keys import add_key
...
def build_invoice_items(
    ...
    *,
    ...
    for row in rows:
        ...
        # export still runs, and SAY SO rather than silently mis-keying. (This
        # comment promised the warning for four releases without emitting one.
        # The stake is real: `idempotency_key` is derived from `kh`, so two
        # keys sharing a 12-hex prefix collapse onto ONE Stripe idempotency
        # key, and Stripe answers the second customer's invoice item with the
        # first one's -- a silent under-bill, not an error.)
        kh = row.get("key_hash")
        if not kh:
            warnings.warn(
                f"export row for key_id={kid} carries no `key_hash` column "
                f"(a pre-0.1.2 export). Falling back to the 12-hex key_id as "
                f"the billing identity and the idempotency key -- that prefix "
                f"is 48 bits, so two keys sharing it would collapse into one "
                f"invoice line. Re-run `meter.export()` on 0.1.2+ to get the "
                f"full sha256.", stacklevel=2)
            kh = kid
        used = row.get("used", 0)
...
def _selftest() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        kh_a, kh_b = (arcaeon.save.meter.key_hash(secret_a),
                      arcaeon.save.meter.key_hash(secret_b))
