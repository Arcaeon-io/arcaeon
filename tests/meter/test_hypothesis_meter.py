...
def test_full_hash_collision_shape_never_merges_counts(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        _write_raw_keys(kp, doc)
        meter = Meter(kp)
        from arcaeon.save.meter import _utc_month
        con = meter._connect()
        try:
...
def test_installed_ledger_version_is_the_fixed_one():
    ...
    major, minor, patch = (int(x) for x in arcaeon.record.ledger.__version__.split(".")[:3])
    assert (major, minor, patch) >= (0, 5, 6), (
        f"arcaeon-ledger {arcaeon.record.ledger.__version__} predates the 0.5.6 "
