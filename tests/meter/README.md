...
`malformed_key`, `no_cap_configured`, `keys_unreadable`) — catch it and answer
however your surface answers. One of those deserves a sentence:
`keys_unreadable` means the keys file exists but could not be READ — a parse
failure or an OS-level fault (PermissionError, a scanner/backup holding the
file), anywhere on the read path including `stat()`. It is deliberately not
`unknown_key`: an unreadable authorization record is a server-side fault, never
"this customer's key is invalid," and never an empty roster.
...
meter.export(month="2026-08")  # a specific month; must be exactly "YYYY-MM"
```

**`month` is validated (0.1.6).** The stored month is exactly what the meter
writes, and every read is an equality match, so a month string that is off by
one character — `"2026-8"`, a trailing space, a `"2026/08"` out of your own
date formatting — used to match nothing and hand back the *complete* roster
with `used: 0` on every line. A clean invoice for nothing, no error, no
warning. Anything that isn't `YYYY-MM` now raises `ValueError`. And an
`export()` whose **keys file does not exist** (a typo, an unmounted volume, a
container rebuilt without the persistent path) now warns rather than returning
an empty invoice that looks exactly like a quiet month.
...
around **20M keys** (about 40% at 16.7M), which is inside this product's own "millions of agents" pitch,
