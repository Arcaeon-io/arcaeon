...
from arcaeon.record.ledger import (Ledger, chain_at, verify_artefact, verify_file,
...
def test_fabricated_prepend_strict_hard_fails_cli_and_mcp(capsys):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        from arcaeon.record.ledger.mcp_server import TOOLS
...
def test_verify_artefact_refetch_mismatch_carries_verdict_tag(monkeypatch):
    ...
    import arcaeon.record.ledger.artefact as artefact_mod
...
# --------------------------------------------------------------------------
# 2026-09-01 (sell-code audit, found from arcaeon-audit's side): a line nested
# 100k deep overflows the C JSON decoder and escapes as RecursionError, which
# every `except ValueError` walked past. verify_file, __iter__, chain_at,
# _last_chain and the MCP stdin loop all crashed on ONE planted line. A verifier
# a tamperer can silence with one line is not a verifier: it is a break.
# --------------------------------------------------------------------------

_DEEP = "[" * 100_000 + "]" * 100_000


def test_pathological_nesting_is_a_break_not_a_crash():
    with tempfile.TemporaryDirectory() as d:
        p = Path(d) / "log.jsonl"
        led = Ledger(p)
        led.append({"a": 1})
        ...
        r = verify_file(p)                       # must not raise
        assert not r.ok
        assert r.first_break == "line 2: unparseable", r.first_break
        assert r.rows == 1
        assert len(list(led)) == 1                # __iter__ skips it, does not raise
        assert chain_at(p, 2) is None            # must not raise
        c2 = led.append({"b": 2})                # _last_chain walks back over it
        assert chain_at(p, 2) == c2


def test_mcp_stdin_loop_outlives_a_pathologically_nested_line():
    import io, sys
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        sys.stdin = io.StringIO(_DEEP + "\n" + good + "\n")
        sys.stdout = io.StringIO()
        try:
            rc = mcp_server.main(["--log", str(p)])
            out = sys.stdout.getvalue()
        finally:
            sys.stdin, sys.stdout = old_in, old_out
        assert rc == 0
        assert '"id": 7' in out, out[:200]        # the call AFTER the bomb was answered
