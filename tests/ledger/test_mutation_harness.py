...
import arcaeon.record.ledger
from arcaeon.record.ledger import VerifyResult, mutation_harness
...
def test_harness_goes_red_when_a_verifier_is_decoration():
    ...
    real = arcaeon.record.ledger.verify_file
    arcaeon.record.ledger.verify_file = lambda path: VerifyResult(ok=True, rows=6, chained=6)
    try:
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            rc = mutation_harness.run()
        ...
        arcaeon.record.ledger.verify_file = real
...
def test_harness_catches_a_truncated_chain_comparison():
    ...
    def prefix_only(path):
        ...
        if r.first_break and "chain mismatch" in r.first_break:
            ...
            prev = arcaeon.record.ledger._GENESIS
            ok = True
            for raw in lines:
                if not raw.strip():
                    continue
                ...
                if claimed is None:
                    continue
                if claimed[:8] != arcaeon.record.ledger._chain(prev, obj)[:8]:
                    ok = False
                prev = claimed
            if ok:
    ...
    arcaeon.record.ledger.verify_file = prefix_only
    try:
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            rc = mutation_harness.run()
...
import arcaeon.record.ledger.mutation_harness as _mh
