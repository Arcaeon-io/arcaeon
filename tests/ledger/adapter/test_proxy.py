...
from arcaeon.record.adapter.observer import FrameSplitter, SeamObserver
from arcaeon.record.adapter.proxy import FAULT_ENV, _Corruptor, _server_label, relay
...
def test_dash_m_package_shorthand_is_equivalent(tmp_path):
    """`python -m arcaeon.record.adapter` is what goes in a config file; it must work."""
    argv = [sys.executable, "-m", "arcaeon.record.adapter", "--ledger",
...
def test_spawn_failure_stderr_does_not_leak_the_unredacted_command(tmp_path):
    """2026-09-05 audit finding. `session_begin` was already careful to log
    only the redacted argv (`safe_command`); the spawn-failure branch's OWN
    stderr line, written straight to the client's error stream, echoed the
    ORIGINAL, unredacted `command` instead -- the exact bug class 0.5.8 fixed
    for `session_begin`, recurring on a path that fix never reached. A command
    line carrying a live credential (the whole reason `_redact_argv` exists)
    leaked it to stderr the moment the wrapped server failed to start."""
    ledger = tmp_path / "leak.jsonl"
    secret = "myrealsecretvalue123"
    cmd = [sys.executable + "-does-not-exist-xyz", "--api-key", secret]
    p = _proxied(b"", ledger, cmd=cmd)
    assert p.returncode == 127
    assert secret.encode() not in p.stderr, p.stderr
    assert b"<redacted>" in p.stderr, p.stderr


def test_no_command_is_a_usage_error(tmp_path):
    p = subprocess.run(
        [sys.executable, "-m", "arcaeon.record.adapter.proxy", "--ledger",
...
def test_session_begin_records_the_wrapped_command_and_backend(tmp_path):
    ...
    assert begin["command"][-1] == "arcaeon.record.adapter._echo_server"
...
def test_server_label_defaults_and_override(tmp_path):
    assert _server_label(["python", "-m", "arcaeon.record.ledger.mcp_server", "--log", "x"]) \
        == "arcaeon.record.ledger.mcp_server"
...
def test_observe_failures_are_reported_in_session_end(tmp_path, monkeypatch):
    ...
    from arcaeon.record.adapter import proxy as proxy_mod
