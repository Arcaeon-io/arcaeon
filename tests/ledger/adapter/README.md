...
itself through the ledger), trimmed for width. Captured on adapter 0.1.0 with
ledger 0.5.7; a current run stamps the installed versions in `seam_impl` and
`ledger_backend`:
...
- anything the operator who runs the proxy chose to change: the proxy is run by the
  same party as the agent, so it is not an outside witness. It proves the agent could
  not skip or edit a row, not that the operator could not.
...
either. Output abridged (two header lines and the tail of each PASS line cut;
the `drop_byte` length differs by a byte or two between runs):
...
### HTTP MCP servers: `--http-forward`

For an agent that talks to an MCP server over HTTP there is no pipe to sit in,
so the adapter opens a local listener and the agent is pointed at it instead:

```
arcaeon-adapter --ledger seam.jsonl --tape agent.tape.jsonl     --http-forward https://tools.example.com/mcp --listen 127.0.0.1:8765
# agent config: MCP URL = http://127.0.0.1:8765/
```

Every request goes to the upstream; every answer (JSON, a batch, or an SSE
stream) comes back with the same status, headers and body bytes, streamed as it
arrives. Hop-by-hop headers are this hop's own, and a chunked or
close-delimited body is re-chunked (same bytes, new framing). Each `tools/call`
gets a seam row (`seam: "mcp-http"`) and a tape row with the same digests the
stdio path and arcaeon-receipt's `call_proxy` write, so `arcaeon-ledger
reconcile` works across the two. A `202` leaves the call open for an answer on a
later stream in the same `Mcp-Session-Id`; an unreachable upstream is a plain
`502` and an `unanswered` row. Ctrl+C (or SIGTERM / Ctrl+Break) ends the
session: `session_end`, then the pin if `--pin-witness` is set.
`--upstream-timeout SECONDS` (default 300) bounds each upstream read, including
an idle SSE stream. No server command is given in this mode.
...
**No network calls of its own at runtime.** The stdio proxy makes none. The
HTTP forward mode makes exactly the calls the agent sends it, to the upstream
the operator named, and `--pin-witness` makes one POST at session end to the
witness the operator named.
...
publication, and harness-hook seams. The HTTP seam exists as a forward mode
(`--http-forward`), not yet as a gateway in front of many upstreams. See `CHANGELOG.md`.
