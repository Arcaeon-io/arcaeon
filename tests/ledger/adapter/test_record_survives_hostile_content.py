...
from arcaeon.record.adapter.observer import SeamObserver, _escape_unencodable
from arcaeon.record.adapter.proxy import FAULT_ENV, REDACTED, _redact_argv
...
def test_ordinary_commands_are_not_touched(tmp_path):
    ...
    argv = [sys.executable, "-m", "arcaeon.record.ledger.mcp_server", "--log", "a.jsonl"]
    out, hits = _redact_argv(argv)
...
def test_session_begin_row_carries_no_secret_but_still_pins_the_command(tmp_path):
    ...
    from arcaeon.record.adapter.proxy import _digest
