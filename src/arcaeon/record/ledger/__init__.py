...
__version__ = "0.8.0"
__all__ = ["LedgerWriteError", "UnverifiedLedgerError", "Ledger", "VerifyResult",
           "verify_file", "chain_at", "authority", "Head",
           "declare_break",
           "bind_artefact", "verify_artefact", "digest_bytes", "digest_json",
           "WitnessStore", "publish_head", "verify_against_witness", "WitnessVerdict"]

# The row format (chain rule, genesis, readers) lives in arcaeon.record.row;
# the private names below are kept because the ledger's own code and tests use them.
from arcaeon.record.row import (  # noqa: E402
    CHAIN_LEN as _CHAIN_LEN, GENESIS as _GENESIS, DuplicateKeyError,
    _reject_duplicate_keys, chain as _chain, loads as _loads,
    loads_strict as _loads_strict)


def _absent(path: Path) -> bool:
    """True only if the file does not exist — a log not yet written to.

    Deliberately NOT `Path.exists()`: that swallows every OSError and answers
    False, so a permission-denied or otherwise unreadable path would be graded
    "brand new" and allowed to mint a genesis pin. Only FileNotFoundError is
    the never-written state; anything else the filesystem says is a real
    problem and stays a red verdict.
    """
    try:
        path.stat()
        return False
...
class UnverifiedLedgerError(ValueError):
    """A publishable pin was refused because the log does not verify (0.7.6).

    Raised by `Head.as_pin()` and `publish_head()`. A pin is the one artefact
    that leaves your control, and its whole meaning is "this is the log, at
    this height, at this time" — minting one over a file whose chain is broken
    publishes a claim the file cannot support, and the damage travels under the
    library's own signature.

    Subclasses ValueError so callers already wrapping `publish_head` (which has
    refused zero-row pins with a ValueError since 0.6.x) keep catching it.
    """
...
class Head:
    ...
    CARRIES THE VERDICT (0.7.6). `head()` verifies the file to count rows and,
    until 0.7.6, threw that verdict away: a log containing one unparseable line
    produced `Head(chain='genesis', rows=0, ...)` — byte-identical to the head
    of a log that had never been written to — and `as_pin()` minted a clean
    genesis pin over the damage. "This log is broken" and "this log is new" must
    not print the same in a tamper-evidence tool, least of all on the artefact
    that gets published somewhere outside your control.

    `ok` mirrors `VerifyResult.ok` and is three-valued for the same reasons
    (see that class): True = every row checked and the chain holds; None =
    no break found but the scan was bounded (skipped prechain rows, or a
    declared break); False = a break was found, and `first_break` names it.
    Both fields default to the green so every existing positional or keyword
    construction of `Head` keeps working unchanged.
    """
    chain: str
    rows: int
    as_of: str
    #: The verify verdict for the file this head was read from. Defaults True
    #: for hand-built heads (a caller asserting a pin they already trust).
    ok: bool | None = True
    #: The first break verify() found, when `ok` is False. None otherwise.
    first_break: str | None = None

    def as_pin(self) -> str:
        """One-line, copy-pasteable pin string for publishing externally.

        REFUSES A PIN OVER A LOG THAT DOES NOT VERIFY (0.7.6): raises
        `UnverifiedLedgerError` when `ok` is False. Repair or declare the break
        first (`declare_break`) — a pin is a published claim, and this one would
        be false.

        A BOUNDED verdict (`ok is None` — skipped prechain rows, or a declared
        break) still pins, deliberately. Bounded is not broken: the height and
        the tip are accurate, no break went unexplained, and refusing here would
        permanently un-pinnable any log that ever used `declare_break`, which
        would punish the honest repair and reward hiding the break instead.
        """
        if self.ok is False:
            raise UnverifiedLedgerError(
                f"refusing to mint a pin over a ledger that does not verify: "
                f"{self.first_break or 'chain broken'}. A pin is published "
                f"outside your control and asserts this log at this height; "
                f"over a broken chain it asserts something the file cannot "
                f"support. Repair the log, or name the break with "
                f"declare_break(), then pin.")
...
class Ledger:
    ...
    def head(self) -> Head:
        ...
        DOES NOT RAISE, ON PURPOSE (0.7.6). The verdict now rides on the
        returned `Head` (`ok`, `first_break`) instead of being discarded, but
        `head()` stays a read: `verify_against_witness()`, the MCP server's
        `head_hash` field, dashboards and the selftest all call it, and a
        damaged log is exactly when they must keep answering rather than
        exploding. The refusal lives one step later, at the point where the
        head becomes a PUBLISHED claim — `as_pin()` and `publish_head()`.
        """
        res = verify_file(self.path)
        ok, first_break = res.ok, res.first_break
        # A log with nothing in it is not a damaged log, and the two verdicts
        # that mean "nothing here" must not be mistaken for damage:
        #   - absent file: verify_file() has no file to read and reports
        #     ok=False "unreadable: [Errno 2] ...". Only FileNotFoundError
        #     counts — a permission error or a directory in the way is a real
        #     red and stays red, which Path.exists() would have hidden.
        #   - zero rows: verify_file() returns ok=None / verified_scope="empty"
        #     (0.5.8, "absence of evidence is not evidence of tampering").
        # Both are the genuine pre-first-append state, and both must still pin
        # as genesis or a caller anchoring a fresh log breaks on day one. A file
        # whose only line is unparseable does NOT land here: it has 0 rows but
        # ok=False and scope "full", so it stays red.
        if ok is False and _absent(self.path):
            ok, first_break = True, None
        elif res.verified_scope == "empty":
            ok, first_break = True, None
        return Head(chain=self._last_chain(), rows=res.rows, as_of=_now_iso(),
                    ok=ok, first_break=first_break)
...
def verify_file(path: str | Path, *, strict: bool = False) -> VerifyResult:
    ...
    for i, raw in enumerate(lines, 1):
        ...
        try:
            ...
            # The bytes are ambiguous: last-wins and first-wins readers see
            # different rows. That is a break, named. The chain walk then
            # continues over the last-wins parse so later rows are still
            # counted honestly instead of cascading from this one.
            res.ok = False
            res.breaks += 1
            res.first_break = res.first_break or f"line {i}: {e}"
            obj = _loads(raw)
        except ValueError:
            res.ok = False
            res.breaks += 1
...
from arcaeon.record.ledger.artefact import (  # noqa: E402
    bind_artefact, verify_artefact, digest_bytes, digest_json,
)
from arcaeon.record.ledger.witness import (  # noqa: E402
    WitnessStore, publish_head, verify_against_witness, WitnessVerdict,
)
