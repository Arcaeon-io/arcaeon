...
attestation `subject`, ready to drop into a ledger row (chain it like any field):
    ...
    from arcaeon.record.ledger import Ledger, bind_artefact
...
# json-c14n v1 canonicalization and the two self-describing digests are the
# row format's, so they come from arcaeon.record.row (one copy).
from arcaeon.record.row import canon_json as _canon_json  # noqa: E402
from arcaeon.record.row import digest_bytes, digest_json  # noqa: E402,F401
...
def _strip_userinfo(url: str) -> str:
    """Drop embedded `user:pass@` userinfo from a URL before it is stored.

    2026-09-05 audit finding: `bind_artefact` stored the URL verbatim as
    `subject.name` and `source_meta.final_url`, and both fields are designed to
    be copied byte-for-byte into a ledger row and, from there, into
    `build_bundle`'s shareable evidence bundle. A URL of the form
    `https://user:pass@host/...` is a live credential — the module's own
    docstring names it a security tradeoff to hash the artefact's bytes, not to
    also carry the fetch credential along for the ride. Same shape as
    `_redact_argv` in the adapter: the record must not hold what the action
    needed. The FETCH itself still uses the original, credentialed URL passed
    in by the caller — only the stored copy is scrubbed.
    """
    try:
        parts = urllib.parse.urlsplit(url)
    except ValueError:
        return url  # unparseable is not this function's problem to solve
...
def bind_artefact(source: Any, *, canon: str = "auto",
    ...
    HONEST LIMIT (2026-09-05): a URL's `user:pass@host` userinfo, if present, is
    used to FETCH but never STORED — `subject.name` and `source_meta.final_url`
    carry the scrubbed URL, because both are designed to be copied verbatim into
    a ledger row and from there into `build_bundle`'s shareable evidence bundle,
    and a live credential has no business riding along in either. The
    consequence: `verify_artefact(..., refetch=True)` re-fetches the SCRUBBED
    URL, so a credentialed resource reports `live_unavailable`, not a false
    `live_match` and not a leaked secret. Don't put live credentials in a URL
    you intend to bind; this only limits how far the mistake travels.
    ...
    if isinstance(source, str) and _looks_like_url(source) and canon != "json-c14n":
        # Fetch with the ORIGINAL, possibly-credentialed URL; store the
        # scrubbed one. See `_strip_userinfo`.
        name = _strip_userinfo(source)
        ...
        with urllib.request.urlopen(req, timeout=30) as r:
            ...
            source_meta = {
                ...
                "final_url": _strip_userinfo(r.geturl()),
            }
