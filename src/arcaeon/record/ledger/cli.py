...
"""ledger CLI - verify or append from the command line.
    ...
    python -m arcaeon.record.ledger.cli verify agent.log.jsonl
    python -m arcaeon.record.ledger.cli verify --strict agent.log.jsonl
    python -m arcaeon.record.ledger.cli append agent.log.jsonl '{"tool":"search","ok":true}'
    python -m arcaeon.record.ledger.cli reconcile agent.tape.jsonl tool.tape.jsonl [--pin pin.json]
    python -m arcaeon.record.ledger.cli --help | --version

Exit codes for `verify` (wire it into CI or a pre-ship gate). The words are
the ones every Arcaeon checker prints; the JSON report's fields are unchanged:
  0 = VERIFIED — EVERY row checked, chain intact.
  1 = BROKEN (a break was found), or bad usage.
  3 = COULD NOT LOOK at every row: no break found in the rows checked, but
      unchained `prechain` rows were skipped (non-strict mode), so this is
      not VERIFIED. The JSON report says how
...
Exit codes for `reconcile` (two tapes and a counter; see arcaeon_ledger.reconcile):
  0 = MATCHED n of n.   1 = MISSING at call k, or ALTERED at call k.
  2 = COULD NOT LOOK (a tape unreachable, both empty, not a tape, pin unreadable).
...
from arcaeon.record.ledger import Ledger, __version__, verify_file
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == "reconcile":
        from arcaeon.prove.reconcile import main as reconcile_main
