...
* **Duplicate ids pair FIFO, and a call never overwrites another.** THE RULE,
  identical in arcaeon-receipt's call_proxy so the two tapes agree: open calls
  are queued per (scope, id) in send order; an answer pairs with the OLDEST open
  call of its id; every sent call gets exactly one tape row and one seam row;
  a call still unpaired when the session ends is `unanswered`. Reusing an id
  violates JSON-RPC, but a completeness recorder must never lose a row over it:
  the old map overwrote the first call, which then had no seam row at all and
  held its tape row open until session end while the second call took the
  first call's answer.
* **An invalid id is rowed, never paired.** JSON-RPC 2.0 says an id is a
  string, a number, or null. A `tools/call` whose id is anything else (true,
  false, an object, an array) was still sent, so it gets one seam row and one
  tape row with `status="invalid_id"` and no result, and no answer pairs to it.
  Same rule in arcaeon-receipt's call_proxy (the old observer dropped it while
  call_proxy taped it, so a clean stream reconciled MISSING on the agent side).
...
from .tape import INVALID_ID, render_invalid_id, valid_rpc_id
...
def _parse(raw: bytes) -> list[dict]:
    ...
    try:
        ...
        # RecursionError (a frame nested past the interpreter's limit) is not
        # a ValueError. It used to escape observe_client_frame before the
        # pending map was touched — the call was forwarded but left no row
        # and no `unanswered` at flush (2026-09-01 audit, adapter #3).
        return []
...
class _Pending:
    __slots__ = ("tool", "args_digest", "args", "t0", "rpc_id", "tape_idx", "key", "n")

    def __init__(self, tool, args_digest, args, t0, rpc_id, tape_idx=None, key=None, n=0):
        self.tape_idx = tape_idx
        self.key = key      # the (scope, id) queue it waits in
        self.n = n          # open order, for a deterministic flush
        self.tool = tool
...
class SeamObserver:
    ...
    def __init__(self, emit: Callable[[dict], Any], *, server: str,
                 ...
                 tape=None, seam: str = SEAM):
        self._emit = emit
        #: Provenance tier on every row: "mcp-stdio" for the pipe, "mcp-http"
        #: for the HTTP forward mode (`http_forward.py`).
        self.seam = seam
        #: Optional `tape.TapeWriter`: the per-call tape `arcaeon_ledger.reconcile`
        #: lines up against the other side's. Never allowed to cost the seam row.
        self.tape = tape
        ...
        #: (scope, id) -> open calls of that id, OLDEST FIRST. A list, not one
        #: slot: see "Duplicate ids pair FIFO" in the module docstring.
    ...
    def observe_client_frame(self, raw: bytes, scope: str | None = None) -> list:
        """Client -> server. Already relayed; this only reads a copy.

        Returns a handle per `tools/call` this frame opened, in order (the HTTP
        forward mode hands them to `close_unanswered` when their exchange ends
        with no answer). A handle names ONE call, so two calls sharing an id are
        closed separately. `scope` keeps two HTTP sessions' identical ids apart;
        the stdio path has one session per pipe and passes none.
        """
        opened: list = []
        for msg in _parse(raw):
            method = msg.get("method")
            key = _scoped(scope, _id_key(msg.get("id")))
            if method == "tools/call":
                if not valid_rpc_id(msg.get("id")):
                    self._invalid_call(msg)
                    continue  # rowed and taped `invalid_id`; no answer pairs to it
                if key is None:
                ...
                with self._lock:
                    ...
                    pend = _Pending(
                        ...
                        t0=self._clock(),
                        rpc_id=msg.get("id"),
                        tape_idx=self._tape_open(params, msg.get("id")),
                        key=key,
                        n=self._opened,
                    )
                    self._enqueue(key, pend)
                    self.calls_seen += 1
                    opened.append(pend)
    ...
    def observe_server_frame(self, raw: bytes, scope: str | None = None) -> None:
        ...
        for msg in _parse(raw):
            ...
            with self._lock:
                ...
                if pend is not None:
                    self._tape_close(pend.tape_idx, msg)
    ...
    def _invalid_call(self, msg) -> None:
        """A `tools/call` whose id is not a string, number or null (true, false,
        an object, an array). JSON-RPC 2.0 makes it invalid, but it was SENT, so
        a completeness recorder rows it: one seam row and one tape row, status
        `invalid_id`, no result digest, never paired (an answer carrying such an
        id is not a response to anything; `_id_key` gives it no key).
        arcaeon-receipt's call_proxy tapes it the same way, so both tapes agree."""
        params = msg.get("params") or {}
        if not isinstance(params, dict):
            params = {}
        args = params.get("arguments")
        ...
        with self._lock:
            ...
            if self.tape is not None:
                try:
                    self.tape.invalid_call(params, msg.get("id"))
                except Exception:
                    self.tape_failures += 1
            self._row(
                "tool_call",
                tool=tool if isinstance(tool, str) else None,
                rpc_id=render_invalid_id(msg.get("id")),
                args_digest=_safe_digest(args),
                status=INVALID_ID,
                ms=0,
                args=args if self.raw else None,
    ...
    # -- the pairing queue (caller holds self._lock) -------------------------

    def _enqueue(self, key, pend) -> None:
        """Queue a call behind any open call of the same (scope, id). Never
        replaces one: the break arm in the tests swaps this for the old
        overwrite and the duplicate-id checks go red."""
        self._pending.setdefault(key, []).append(pend)

    def _take_oldest(self, key):
        """The oldest open call of this (scope, id), removed; None if none."""
    ...
    def _take(self, pend) -> bool:
        """Remove THIS call if it is still open. False if it was already paired."""
        q = self._pending.get(pend.key)
        if not q:
            return False
    ...
    # -- the tape (two tapes and a counter) ----------------------------------

    def _tape_open(self, params, rpc_id):
        if self.tape is None:
            return None
        try:
            return self.tape.open_call(params, rpc_id)
        except Exception:
            self.tape_failures += 1
            return None

    def _tape_close(self, idx, msg):
        if self.tape is None or idx is None:
            return
        try:
            self.tape.close_call(idx, msg)
        except Exception:
            self.tape_failures += 1

    def close_unanswered(self, handles, *, reason: str) -> int:
        """Row as `unanswered` each call in `handles` (from `observe_client_frame`)
        that is still pending. Returns how many.

        For a transport where an exchange can END without the answer (an HTTP
        response that carried none, an upstream that could not be reached): the
        call is closed then, on the seam and on the tape, instead of holding every
        later tape row behind it until the session ends. Calls already answered
        are not in the pending map and are left alone.
        """
        n = 0
        with self._lock:
            for pend in handles:
                if not isinstance(pend, _Pending) or not self._take(pend):
                    continue
                n += 1
                self._tape_close(pend.tape_idx, None)
                self._row(
                    "tool_call",
                    tool=pend.tool,
                    rpc_id=_render_id(pend.rpc_id),
    ...
    def flush_pending(self, *, reason: str = "no_response") -> int:
        ...
        """
        with self._lock:
            items = sorted((p for q in self._pending.values() for p in q),
                           key=lambda p: p.n)
            self._pending.clear()
            for pend in items:
                self._row(
...
def _scoped(scope: str | None, key: str | None):
    """Prefix a pending key with its session scope (HTTP forward mode only)."""
    if key is None or not scope:
        return key
    return (scope, key)  # a tuple: no separator a header value could forge
