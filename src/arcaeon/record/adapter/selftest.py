...
"""Selftest + mutation harness: every claimed check, observed actually failing.
    ...
    python -m arcaeon.record.adapter.selftest
...
_ECHO_CMD = [sys.executable, "-m", "arcaeon.record.adapter._echo_server"]
...
def _child_env(fault: str | None = None) -> dict:
    ...
    env = dict(os.environ)
    # the directory that holds the top-level `arcaeon` package (src/ in a checkout)
...
def _run_proxied(ledger: Path, fault: str | None = None, extra: list | None = None) -> bytes:
    ...
    cmd = [sys.executable, "-m", "arcaeon.record.adapter.proxy", "--ledger", str(ledger)]
    cmd += list(extra or [])
    cmd += ["--"] + _ECHO_CMD
