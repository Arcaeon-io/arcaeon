...
Three calls:
    ...
    from arcaeon.prove.compact import CompactionReceipt, verify_receipt
...
def _well_formed(digest: str) -> bool:
    ...
    The tail is checked against an explicit lowercase-hex allowlist, not
    `int(tail, 16)`: that call also accepts `0x1f`, `+1f`, ` 1f` and `1_f`,
    none of which any digest recipe ever emits (disclosed 0.1.3, fixed 0.1.5).
    """
    parts = digest.split(":") if isinstance(digest, str) else []
    if len(parts) != 4 or not all(parts):
        return False
    return all(c in _HEX for c in parts[3])


def _is_count(value: Any) -> bool:
    """A count or byte total: a non-negative int, and not a bool (JSON `true`
    is not a number of bytes, even though Python lets it subtract as 1)."""
...
class CompactionReceipt:
    ...
    -> `seal(...)`. After seal the receipt refuses further writes; recording or sealing
...
def verify_receipt(row: dict, pre_content: Sequence[Any] | None = None,
    ...
    receipt_digest reproduces from the row's own core — so an edit to the
    CORE (schema, pre, post, dropped, introduced, compactor, method) is
    caught even when the row has been copied out of its ledger. The
    timestamps (`ts`, `opened_at`) and `kind` are NOT in the core: out of the
    ledger they are unattested, and a backdated copy passes this function.
    Inside the ledger the chain covers the whole row, timestamps included.
    ...
    Returns:
        {"ok": bool,                       # self_consistent AND content didn't mismatch
         ...
         "verified_scope": "full" | "bounded_pre_only" | "bounded_post_only"
                           | "bounded_no_content" (any bounded value may carry
                           a "+v1_lower_bound" suffix) | "none" (early return),
         "notes": [<str>, ...]}

    READ `verified_scope`, NOT ONLY `ok` (0.1.4). `ok` says nothing you looked
    at was wrong; `verified_scope` says how much you looked at. `ok=True` with
    `verified_scope="bounded_no_content"` means the row's numbers are mutually
    coherent and NOTHING was recomputed -- a receipt understating what it
    destroyed passes that check by construction, and on a v1 row claiming an
    introduction it can do so by an arbitrary margin. Only
    `verified_scope="full"` means every claimed number was recomputed from
    content you hold. This mirrors arcaeon-ledger's rule that a scan which did
    not cover everything is not a green.
    ...
    out = {"ok": False, "self_consistent": False, "content": "skipped",
           "schema": None, "understatement_check": None,
           "verified_scope": "none", "notes": notes}

    # --- self-consistency ---------------------------------------------------
    # A row this function cannot even read is a verdict (ok=False, "malformed
    # receipt row"), never a traceback (0.1.5). A caller sweeping a ledger
    # with one hostile row in it must get an answer for that row and keep
    # going -- an exception mid-sweep is a verifier that can be switched off
    # by the thing it verifies.
    sc = True
    if not isinstance(row, dict):
        notes.append(f"malformed receipt row: expected a JSON object, got "
                     f"{type(row).__name__}")
        return out
    ...
    try:
        ...
        return out
    # Shape gate. Everything below does arithmetic on the counts and hashes
    # the manifest entries; a string count or an object in the manifest used
    # to surface as TypeError from inside those lines. Refuse here, plainly.
    if not all(_is_count(c) for c in counts):
        notes.append("malformed receipt row: counts/bytes must be non-negative "
                     "integers")
        return out
    if not isinstance(manifest, list) or \
            not all(isinstance(d, str) for d in manifest):
        notes.append("malformed receipt row: dropped.items must be a list of "
                     "digest strings")
        return out
    ...
    # --- how much of the claim was actually CHECKED --------------------------
    # `ok` answers "did anything I looked at contradict the row." It has never
    # answered "how much did I look at," and those are different questions --
    # the gap this package's own sibling (arcaeon-ledger 0.5.7+) refuses to
    # leave open, by returning ok=None for any scan that did not cover
    # everything. Here `ok` stays a plain bool for compatibility, so the
    # coverage answer is carried in its own field, in the ledger's vocabulary:
    #
    #   "full"                  both contents supplied and reconciled -- every
    #                           claimed number was recomputed from content.
    #   "bounded_pre_only"      only pre_content given: the drop SET was never
    #   "bounded_post_only"     recomputed, because that needs both sides.
    #   "bounded_no_content"    self-consistency only. Nothing was recomputed;
    #                           this proves the row's numbers are mutually
    #                           coherent, not that any one of them is true.
    #   "+v1_lower_bound"       appended when a v1 row claims an introduction:
    #                           post.bytes is then only LOWER-bounded, so
    #                           dropped.bytes can be understated behind the
    #                           claimed introduction (the HIGH-1 gap). Closed
    #                           by content recomputation, or by re-sealing v2.
    #
    # A bounded scope is not a failure and does not flip `ok`. It is the
    # difference between "I checked everything and found nothing wrong" and
    # "I found nothing wrong in the part I checked," which a caller reading
    # only `ok` cannot otherwise tell apart.
    if pre_content is not None and post_content is not None:
        scope = "full"
    elif pre_content is not None:
        scope = "bounded_pre_only"
    elif post_content is not None:
        scope = "bounded_post_only"
    else:
        scope = "bounded_no_content"
    v1_gap = (schema_version == "v1" and scope != "full"
              and isinstance(introduced.get("count"), int)
              and introduced["count"] > 0)
    if v1_gap:
        scope += "+v1_lower_bound"
    out["verified_scope"] = scope

    # The silence was the defect: a bounded pass returned an EMPTY notes list,
    # which reads exactly like a clean full verification. Say what was skipped.
    if scope != "full":
        if scope.startswith("bounded_no_content"):
            notes.append("bounded scan: no content supplied, so nothing was "
                         "recomputed -- this is self-consistency only, not "
                         "proof that pre/post/dropped are true "
                         "(verified_scope=%s)" % scope)
        else:
            notes.append("bounded scan: only one side of the content was "
                         "supplied, so the drop set was not recomputed "
                         "(verified_scope=%s)" % scope)
    if v1_gap:
        notes.append("bounded scan: v1 row claiming an introduction -- "
                     "post.bytes is only a lower bound, so dropped.bytes "
                     "could be understated behind the claimed introduction "
                     "(HIGH-1). Supply content or re-seal under v2 to pin it.")
    return out
