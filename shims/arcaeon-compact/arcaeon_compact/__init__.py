...
"""arcaeon_compact: compatibility shim. This code now lives in `arcaeon.prove.compact`.
...
_SUBMODULES = {
    'arcaeon_compact.selftest': 'arcaeon.prove.compact.selftest',
}

#: the old package's own __all__, unchanged
__all__ = ['CompactionReceipt',
           'verify_receipt',
           'SCHEMA',
           'SCHEMA_V1',
           'SCHEMA_V2']
...
class _Alias(_abc.MetaPathFinder, _abc.Loader):
    """`import arcaeon_compact.x` returns the new module itself, not a copy."""
    ...
    def get_code(self, name):          # `python -m arcaeon_compact.x` (runpy)
