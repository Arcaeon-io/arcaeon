...
"""arcaeon_audit: compatibility shim. This code now lives in `arcaeon.prove.audit`.

`pip install arcaeon-audit` installs `arcaeon` and this one forwarding module, nothing
else. Every old name (and every old submodule, `arcaeon_audit.<name>`) resolves to
...
_SUBMODULES = {
    'arcaeon_audit.cli': 'arcaeon.prove.audit.cli',
}
...
_warnings.warn("arcaeon_audit is deprecated: import arcaeon.prove.audit instead (removed in arcaeon 1.0.0)",
...
class _Alias(_abc.MetaPathFinder, _abc.Loader):
    """`import arcaeon_audit.x` returns the new module itself, not a copy."""
    ...
    def get_code(self, name):          # `python -m arcaeon_audit.x` (runpy)
...
def _script_arcaeon_audit(argv=None):
    """The old `arcaeon-audit` console script: now `arcaeon audit --legacy-exit`."""
    argv = list(_sys.argv[1:] if argv is None else argv)
    from arcaeon.cli import main
    return main(["audit", *argv, "--legacy-exit"])
